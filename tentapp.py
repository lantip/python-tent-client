...
def buildHmacSha256AuthHeader(mac_key_id, mac_key, method, resource, hostname, port):
    ...
    msg = '\n'.join([str(
        timestamp), nonce, method, resource, hostname, str(port), '', ''])
    debugDetail('input to hash: ' + repr(msg))
    debugRaw(msg)

    digest = hmac.new(
        removeUnicode(mac_key), removeUnicode(msg), hashlib.sha256).digest()
    mac = removeUnicode(
        b64encode(digest).decode())  # this produces unicode for some reason
    authHeader = 'MAC id="%s" ts="%s" nonce="%s" mac="%s"' % (
        removeUnicode(mac_key_id), timestamp, nonce, mac)
...
def retry(method, *args, **kwargs):
    for ii in range(retries):
        try:
            ...
            debugError('http error.  retrying... (that was attempt %s of %s)' %
                       (ii, retries))
...
class KeyStore(object):
    ...
    def _save(self):
        open(self.filename, 'w').write(
            json.dumps(self.keys, sort_keys=True, indent=4) + '\n')
...
class TentApp(object):
    def __init__(self, entityUrl):
        ...
        # if authenticate is run later, this session will be replaced with a
        # session that does authentication
    ...
    def _discoverAPIUrls(self, entityUrl):
        ...
        # Get self.entityUrl doing just a HEAD request so we can get Link
        # headers
        ...
        try:
            ...
            # header.  I think it returns the last one, but we should be using
            # the first one.
            ...
            try:
                ...
                links = [link.split('href="')[1].split('"')
                         [0] for link in links]
                profileUrls = [removeUnicode(link) for link in links]
            except IndexError:
                raise DiscoveryFailure(
                )  # couldn't find Link header or <link> tag
        ...
        # But now we want to get the profileUrl, which is maybe on a different
        # host.
        r = retry(requests.get, profileUrls[0], headers=DEFAULT_HEADERS)
        profile = r.json()
        self.entityUrl = removeUnicode(
            profile['https://tent.io/types/info/core/v0.1.0']['entity'])
        self.apiRootUrls = profile[
            'https://tent.io/types/info/core/v0.1.0']['servers']
    ...
    def register(self):
        ...
        r = retry(requests.post, requestUrl, data=json.dumps(
            appInfoJson), headers=headers)
        ...
        self.keys['registration_mac_key_id'] = r.json(
        )['mac_key_id'].encode('utf-8')
    ...
    def getUserApprovalURL(self):
        ...
        if self.appDetails.get('postNotificationURL', None):
            params['tent_notification_url'] = self.appDetails[
                'postNotificationUrl']
        debugJson(params)
    ...
    def getPermanentKeys(self, code, state=None):
        ...
        # first make the auth headers are using the credentials from the
        # registration step
        ...
        debugRequest('posting to: %s' % requestUrl)
        r = retry(self.session.post, requestUrl, data=json.dumps(
        ...
        self.keys['permanent_mac_key_id'] = r.json(
        )['access_token'].encode('utf-8')
        self.keys['permanent_mac_key'] = r.json()['mac_key'].encode('utf-8')
        debugDetail(
            'permanent mac key_id: %s' % self.keys['permanent_mac_key_id'])
    ...
    def follow(self, entityUrl):
        ...
        r = retry(self.session.post, requestUrl, data=json.dumps({
                  'entity': entityUrl}), headers=headers)
    ...
    def authorizeFromCommandLine(self, keyStoreFilename):
        ...
        # 'http://zzzzexample.com/oauthcallback'
